...
| □ | git status 无意外文件 | 无 `.env` / `credentials.json` / `node_modules` / `.claude/worktrees/` |
| □ | `.gitignore` 覆盖关键目录 | `grep -c "dist2\|\.claude/worktrees\|release" .gitignore` |
...
| □ | **pre-audit.sh §17 通过** (T2406 R352) | `bash scripts/pre-audit.sh` → §17 Collapse Constitution Defense 零警告 |
...
- 检查暂存区无 `.claude/worktrees/` (嵌入仓库) 或临时脚本
- 确保 `.gitignore` 已包含 `dist2/` 和 `.claude/worktrees/`
