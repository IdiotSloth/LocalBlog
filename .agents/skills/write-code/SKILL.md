...
| □ | 无 Ghost Risk: `fixed` panel / expandable section / localStorage UI 状态 / hidden context / 模块级 pub/sub | 防复杂度反弹 | grep `position:\s*fixed\|expandable\|localStorage\.setItem\|display:\s*none\|window\.__` |
