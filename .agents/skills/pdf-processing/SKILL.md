...
This skill provides comprehensive PDF processing capabilities leveraging Codex's native PDF support, which can:
...
Codex should automatically activate this skill when:
...
python .Codex/skills/pdf-processing/scripts/decode_pdf.py --input base64_string.txt --output document.pdf
```
...
python .Codex/skills/pdf-processing/scripts/download_pdf.py --url "https://example.com/doc.pdf" --output document.pdf
```

### Step 2: Read and Analyze PDF

Use the **Read tool** to access PDF files. Codex's native PDF support will:
...
```bash
python .Codex/skills/pdf-processing/scripts/extract_text.py \
  --input document.pdf \
  --output document.txt \
...
```bash
python .Codex/skills/pdf-processing/scripts/extract_tables.py \
  --input document.pdf \
  --format csv \
...
```bash
python .Codex/skills/pdf-processing/scripts/summarize_pdf.py \
  --input document.pdf \
  --output summary.md \
...
```bash
python .Codex/skills/pdf-processing/scripts/convert_pdf.py \
  --input document.pdf \
  --output document.md \
  --format markdown \
...
```bash
python .Codex/skills/pdf-processing/scripts/analyze_visuals.py \
  --input document.pdf \
...
python .Codex/skills/pdf-processing/scripts/ocr_pdf.py \
...
1. **Always use the Read tool first** - This leverages Codex's native PDF support for best results
...
python .Codex/skills/pdf-processing/scripts/batch_process.py \
  --input-dir ./pdfs/ \
...
python .Codex/skills/pdf-processing/scripts/convert_pdf.py \
  --input document.pdf \
  --output document.md \
  --template .Codex/skills/pdf-processing/assets/custom_template.md
```

### Selective Extraction
```bash
# Extract only specific sections
python .Codex/skills/pdf-processing/scripts/extract_sections.py \
  --input document.pdf \
...
- `pdf_capabilities.md` - Detailed breakdown of Codex's PDF support
