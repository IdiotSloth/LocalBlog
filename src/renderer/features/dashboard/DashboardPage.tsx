...
export function DashboardPage() {
  ...
  useEffect(() => {
    if (!user) return;
    let aborted = false;
    window.api.workspaceGetInfo(user.id).then((d: unknown) => {
      if (aborted) return;
      const s = d as Record<string, unknown>;
      if (s && typeof s.blogCount === 'number') setWs(s);
    }).catch(() => {});
    window.api.statsGet(user.id).then((d: unknown) => {
      if (aborted) return;
      const r = d as any;
      if (r.success && r.data) {
        setStats(r.data);
  ...
  return (
    <div style={{ maxWidth: 'var(--content-max)', margin: '0 auto' }}>
      ...
      <div className="mb-8" style={{ fontFamily: 'var(--font-mono)' }}>
        ...
        <h1 className="mt-1 text-[40px] font-bold leading-tight" style={{ color: 'var(--text-primary)' }}>{user?.username || '...'}</h1>
        <p className="mt-1 text-[18px]" style={{ color: 'var(--text-secondary)' }}>本地博客与知识库</p>

        {/* Streak badges */}
        {stats && (stats.currentStreak > 0 || stats.totalWords > 0) && (
          <div className="mt-3 flex flex-wrap gap-2">
            {stats.currentStreak > 0 && <span className="rounded-[12px] px-3 py-1 text-[12px]" style={{ background: 'rgba(211,153,34,0.15)', color: 'var(--accent-amber)' }}>🔥 连续 {stats.currentStreak} 天</span>}
            {stats.totalWords > 0 && <span className="rounded-[12px] px-3 py-1 text-[12px]" style={{ background: 'var(--bg-tertiary)', color: 'var(--text-secondary)' }}>累计 {(stats.totalWords / 1000).toFixed(1)}k 字</span>}
            {stats.totalBlogs > 0 && <span className="rounded-[12px] px-3 py-1 text-[12px]" style={{ background: 'var(--bg-tertiary)', color: 'var(--text-secondary)' }}>{stats.totalBlogs} 篇文章</span>}
          </div>
        )}
      </div>
      ...
      <div className="mb-8 grid grid-cols-4 gap-3">
        {[
          ...
          <div key={card.label} className="rounded-[6px] border p-4" style={{ background: 'var(--bg-secondary)', borderColor: 'var(--border-default)' }}>
            <div className="text-[28px] font-bold" style={{ color: `var(${card.c})` }}>{card.val}</div>
            <div className="mt-1 flex items-center gap-2 text-[12px]" style={{ color: 'var(--text-secondary)' }}>
              {card.label}
              {card.sub && <span className="text-[11px]" style={{ color: 'var(--accent-green)' }}>{card.sub}</span>}
            </div>
          </div>
        ))}
      </div>
      ...
      {/* Heatmap */}
      {user && <div className="mb-8"><Heatmap userId={user.id} /></div>}

      {/* Quick actions */}
      <h3 className="mb-3 text-[14px] font-semibold uppercase tracking-wider" style={{ color: 'var(--text-secondary)' }}>快捷操作</h3>
      <div className="grid grid-cols-4 gap-3">
        {[
          ...
          <Link key={a.to} to={a.to} className="rounded-[6px] border p-4 transition-all duration-[0.2s]" style={{ background: 'var(--bg-secondary)', borderColor: 'var(--border-default)' }}
