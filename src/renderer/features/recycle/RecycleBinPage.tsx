    ...
    <div style={{ maxWidth: 'var(--content-max)', margin: '0 auto' }}>
      <div className="mb-6 flex items-center justify-between">
        <h2 className="text-[24px] font-semibold" style={{ color: 'var(--text-primary)' }}>
          回收站 <span className="text-[14px] font-normal" style={{ color: 'var(--text-secondary)' }}>{items.length} 项</span>
        </h2>
        {items.length > 0 && (
      ...
      {loading ? (
        ...
        <div className="rounded-[6px] border border-dashed p-12 text-center"
          style={{ borderColor: 'var(--border-default)', background: 'var(--bg-secondary)' }}>
          <p className="text-[14px]" style={{ color: 'var(--text-secondary)' }}>回收站为空</p>
        </div>
      ) : (
        <div className="overflow-hidden rounded-[6px] border" style={{ borderColor: 'var(--border-default)', background: 'var(--bg-secondary)' }}>
          <table className="w-full text-[14px]">
            <thead style={{ background: 'var(--bg-tertiary)' }}>
              <tr>
                <th className="px-4 py-3 text-left font-medium" style={{ color: 'var(--text-secondary)' }}>类型</th>
                <th className="px-4 py-3 text-left font-medium" style={{ color: 'var(--text-secondary)' }}>ID</th>
                <th className="px-4 py-3 text-left font-medium" style={{ color: 'var(--text-secondary)' }}>删除时间</th>
              ...
              {items.map((item) => (
                <tr key={item.id} className="border-t transition-colors"
                  ...
                  <td className="px-4 py-3">
                    <span className="rounded-[4px] px-2 py-0.5 text-[11px] font-medium"
                      style={{ color: item.itemType === 'blog' ? 'var(--accent-blue)' : 'var(--accent-green)',
                               background: 'var(--bg-tertiary)' }}>
                      {item.itemType === 'blog' ? '博客' : '文件'}
                    </span>
                  </td>
                  <td className="px-4 py-3 font-mono text-[12px]" style={{ color: 'var(--text-secondary)' }}>#{item.itemId}</td>
                  <td className="px-4 py-3" style={{ color: 'var(--text-secondary)' }}>{formatDate(item.deletedAt)}</td>
