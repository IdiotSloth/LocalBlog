  ...
  return (
    <div>
      <div className="flex flex-wrap items-center gap-2">
        <span className="text-[13px] font-medium" style={{ color: 'var(--text-secondary)' }}>标签:</span>
        {selectedTags.map((t) => (
        ...
        <button ref={triggerRef} type="button" onClick={() => setOpen(!open)}
          ...
          style={{ borderColor: 'var(--border-default)', color: 'var(--text-secondary)', background: open ? 'var(--bg-tertiary)' : 'transparent' }}>
          {open ? '收起' : '+ 标签'}
        </button>
      </div>

      {open && (
        <div className="z-50 w-72 rounded-[6px] border p-3 shadow-lg"
          style={{ ...panelPos, maxHeight: 'min(340px, calc(100vh - 24px))', overflowY: 'auto', background: 'var(--bg-secondary)', borderColor: 'var(--border-default)' }}>
          {/* Quick create */}
          <div className="mb-2 flex gap-2">
            ...
            <button type="button" onClick={createAndAdd} disabled={creating || !newName.trim()}
              className="rounded-[4px] px-2 py-1 text-[12px] font-medium disabled:opacity-30"
              style={{ background: 'var(--accent-blue)', color: 'var(--text-on-accent)' }}>
              创建
            </button>
          </div>
          {error && <p className="mb-2 text-[11px]" style={{ color: 'var(--accent-red)' }}>{error}</p>}

          {/* Tag list */}
          {loading ? <p className="py-2 text-center text-[12px]" style={{ color: 'var(--text-secondary)' }}>加载中...</p>
          : tags.length === 0 ? <p className="py-2 text-center text-[12px]" style={{ color: 'var(--text-secondary)' }}>暂无标签，创建一个吧</p>
            ...
            {tags.map((t) => {
              ...
              return (
                <button key={t.id} type="button" onClick={() => toggle(t.id)}
                  ...
                  style={{ background: sel ? 'var(--bg-tertiary)' : 'transparent', color: sel ? 'var(--accent-blue)' : 'var(--text-primary)' }}>
                  <span>{t.name}</span>
                  <span className="rounded-[3px] px-1.5 py-0.5 text-[11px]" style={{ background: sel ? 'var(--accent-blue)' : 'var(--bg-primary)', color: sel ? 'var(--text-on-accent)' : 'var(--text-secondary)' }}>
                    {t.count ?? 0}
                  </span>
                </button>
              );
