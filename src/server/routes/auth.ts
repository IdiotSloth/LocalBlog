...
authRouter.post('/register', async (req, res) => {
  try {
    ...
    const [result] = await pool.execute(
      'INSERT INTO users (username, password_hash, workspace_path) VALUES (?, ?, ?)',
      [username, hash, workspacePath],
    ) as any[];
    const userId = result.insertId;

    const sessionToken = generateToken();
    const expiresAt = new Date(Date.now() + 30 * 24 * 60 * 60 * 1000);
    await pool.execute('INSERT INTO sessions (user_id, token, expires_at) VALUES (?, ?, ?)', [userId, sessionToken, expiresAt]);
    ...
    return res.json({ success: true, user: { id: userId, username, workspacePath, createdAt: new Date().toISOString() }, token: jwtToken });
  } catch (err) {
...
authRouter.post('/login', async (req, res) => {
  try {
    ...
    await pool.execute('INSERT INTO sessions (user_id, token, expires_at) VALUES (?, ?, ?)', [user.id, sessionToken, expiresAt]);
    ...
    return res.json({ success: true, user: { id: user.id, username, workspacePath: user.workspace_path, createdAt: user.created_at }, token: jwtToken });
  } catch (err) {
    return res.json({ success: false, error: (err as Error).message });
  }
});
...
authRouter.get('/session', requireAuth, async (req: AuthRequest, res) => {
  try {
    ...
    const u = rows[0];
    return res.json({ success: true, user: { id: u.id, username: u.username, workspacePath: u.workspace_path, createdAt: u.created_at } });
  } catch (err) {
    return res.json({ success: false, error: (err as Error).message });
  }
});
