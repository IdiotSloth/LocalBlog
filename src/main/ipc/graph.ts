...
async function resolveNodeLabel(type: string, id: number, userId: number): Promise<string> {
  if (type === 'blog') {
    const rows = await dbAll<{ title: string }>('SELECT title FROM blogs WHERE id = ? AND user_id = ?', [id, userId]);
    return rows[0]?.title ?? `Blog #${id}`;
  }
  if (type === 'knowledge') {
    const rows = await dbAll<{ filename: string }>('SELECT filename FROM knowledge_files WHERE id = ? AND user_id = ?', [id, userId]);
    return rows[0]?.filename ?? `File #${id}`;
  }
  if (type === 'note') {
    const rows = await dbAll<{ title: string }>('SELECT title FROM notes WHERE id = ? AND user_id = ?', [id, userId]);
    return rows[0]?.title ?? `Note #${id}`;
  }
  return `${type} #${id}`;
}

interface TagEdgeRow { tagId: number; name: string }

async function getTagEdgesForNode(srcType: string, srcId: number, userId: number): Promise<TagEdgeRow[]> {
  if (srcType === 'blog') {
    return dbAll<TagEdgeRow>(
