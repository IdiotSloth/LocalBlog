...
export function registerFolderHandlers(): void {
  ...
  ipcMain.handle(IPC.FOLDER_CREATE, async (_event, data: { userId: number; name: string; type: string; parentId?: number }) => {
    try {
  ...
  ipcMain.handle(IPC.FOLDER_MOVE_ITEM, async (_event, data: { itemType: string; itemId: number; folderId: number | null }) => {
    try {
