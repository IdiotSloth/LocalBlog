...
import { getBlogPath, getBlogAssetsDir, getWorkspacePath } from '../utils/paths';

// Raw DB row — snake_case as returned by SQL
interface RecycleRow { id: number; user_id: number; item_type: string; item_id: number; deleted_at: string; }

function rowToItem(r: RecycleRow): RecycleBinItem {
  return { id: r.id, userId: r.user_id, itemType: r.item_type as 'blog' | 'knowledge_file', itemId: r.item_id, deletedAt: r.deleted_at };
}

export class RecycleService {
  static async listItems(userId: number): Promise<RecycleBinItem[]> {
    const rows = await dbAll<RecycleRow>('SELECT id, user_id, item_type, item_id, deleted_at FROM recycle_bin WHERE user_id = ? ORDER BY deleted_at DESC', [userId]);
    return rows.map(rowToItem);
  }

  static async restoreItem(userId: number, itemId: number, itemType: string): Promise<void> {
    const item = await dbGet<RecycleRow>('SELECT * FROM recycle_bin WHERE user_id = ? AND item_id = ? AND item_type = ?', [userId, itemId, itemType]);
  ...
  private static async permanentlyDeleteItem(item: RecycleRow): Promise<void> {
    ...
    if (item.item_type === 'blog') {
      ...
      const blog = await dbGet<{ user_id: number; format: string }>(
        'SELECT user_id, format FROM blogs WHERE id = ?', [item.item_id]);
      if (blog) {
        try { toDelete.push(await getBlogPath(blog.user_id, item.item_id, blog.format as 'md' | 'html')); } catch {}
        try { toDeleteDirs.push(await getBlogAssetsDir(blog.user_id, item.item_id)); } catch {}
      }
      ...
      const kf = await dbGet<{ user_id: number; file_path: string }>(
        'SELECT user_id, file_path FROM knowledge_files WHERE id = ?', [item.item_id]);
      if (kf) {
        try {
    ...
    for (const p of toDelete) { try { if (fs.existsSync(p)) fs.unlinkSync(p); } catch {} }
    for (const d of toDeleteDirs) { try { if (fs.existsSync(d)) fs.rmSync(d, { recursive: true, force: true }); } catch {} }
  }
}
